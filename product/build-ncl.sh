...
pushd /tmp/phantomjs/
  ...
  time wget -q --show-progress http://download.jboss.org/jbosstools/updates/requirements/node/phantomjs/phantomjs-2.1.1-linux-x86_64.tar.bz2
popd
pushd dashboard
...
-DnodeDownloadRoot=${nodeDownloadRoot} -DnpmDownloadRoot=${npmDownloadRoot} -DnpmRegistryURL=${npmRegistryURL}
