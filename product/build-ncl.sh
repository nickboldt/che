...
# fetch latest dashboard or snapshot
if [[ $includeDashboardVersion ]]; then
  pushd /tmp/
    mvn dependency:get -DgroupId=org.eclipse.che.dashboard -DartifactId=che-dashboard-war -Dversion=${includeDashboardVersion} -Dpackaging=war \
     -DremoteRepositories=http://oss.sonatype.org/content/repositories/snapshots/
    mvn dependency:get -DgroupId=org.eclipse.che.dashboard -DartifactId=che-dashboard-war -Dversion=${includeDashboardVersion} -Dclassifier=sources  \
     -DremoteRepositories=http://oss.sonatype.org/content/repositories/snapshots/
    dashboardm2Folder=${HOME}/.m2/repository/org/eclipse/che/dashboard/che-dashboard-war/${includeDashboardVersion}/
    ls -la ${dashboardm2Folder}
  popd
  # get actualdashboard version, eg. 6.12.0-20180921.144339-40 instead of 6.12.0-SNAPSHOT
  cheDashboardVersion=$(cat ${dashboardm2Folder}/maven-metadata-temp.xml | grep value | tail -1 | sed -e "s#.*<value>\(.\+\)</value>#\1#")
fi
